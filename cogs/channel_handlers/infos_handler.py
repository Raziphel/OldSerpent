...
class rules_handler(Cog):
    ...
    async def rules(self):
        ...
        description=f"**Here is some common things about the Serpent bot that every member should know!**\n\nThe Serpent is always rewarding members coins & XP for every message, voice channel and command they run!\nIf you see the @Dead role on any member they lose permission to see Read Message history and sending messages!\nYou can move between different areas of Serpent's Garden by using `/goto` or using the <#{self.bot.config['info_channels']['handbook']}>.", color=0xF5AE47)

        embed4=Embed(title=f"**[- The Handbook -]**", 
        description=f"**We try hardest to not make things complicated,** but for those who wanna know the details about everything going on in Serpent's Garden, please read the <#{self.bot.config['info_channels']['handbook']}>!\n\n**Here are some important tihngs to read!**\n1.)\n2.)\n3.)\n4.)", color=0xB6F547)
