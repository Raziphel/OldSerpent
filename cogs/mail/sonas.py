...
class Sonas(Cog):
    ...
    async def setsona(self, ctx):
        ...
        if ctx.channel.id in self.bot.config['fur-channels'].values(): pass
        elif ctx.channel.id in self.bot.config['nsfw-fur-channels'].values(): pass
        else:
            return
