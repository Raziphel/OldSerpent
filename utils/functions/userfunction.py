...
class UserFunction(object):
    ...
    async def verify_user(cls, user, type):
        ...
        if type == "guild":
            ...
            try:
                await general.send(content=f"<@&1134574155828838560> {user.mention}", embed=utils.SpecialEmbed(description=f"Welcome {user.mention} to my garden!\nYou should visit <id:customize> as there is still a plenty more roles for you to choose from!", thumbnail=user.avatar.url, footer="Enjoy your stay~"))
            except: pass
            return
